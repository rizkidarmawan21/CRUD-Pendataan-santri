...
@section('content_header')
    ...
    @if (Auth::user()->is_admin == 0)
        <h1 class="m-0 text-dark">Kamar Santri
            {!! $kampus ? "&raquo; {$kampus}" : '' !!}
            {!! $gedung ? "&raquo; {$gedung}" : '' !!}
            {!! $kamar ? "&raquo; {$kamar}" : '' !!}
        </h1>
    @else
        <h1 class="m-0 text-dark">Kamar Santri &raquo;
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex">
                        ...
                        @php
                            ...
                            href="' .
                                    route('detail.santri.edit', $item->id) .
                                    '">EDIT</a></nobr>
                    <nobr><a class="badge bg-info border-0"
                            href="' .
                                    route('datasantri.show', $item->santri->id) .
                                    '">DETAIL</a></nobr>
                    <nobr>
                        <form action="' .
                                    route('detail.santri.delete', $item->id) .
                    ...
                    ',
                        ...
                        {{-- With buttons --}}
                        <x-adminlte-datatable id="table5" :heads="$heads" :config="$config" theme="light" striped
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" multiple="multiple" data-placeholder="Pilih Santri"
                                        ...
                                        @foreach ($masterSantri as $mSantri)
                                            <option value="{{ $mSantri->id }}">{{ $mSantri->kampus }} -
                                                {{ $mSantri->nama }} - {{ $mSantri->jenjang }} {{ $mSantri->kelas }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
